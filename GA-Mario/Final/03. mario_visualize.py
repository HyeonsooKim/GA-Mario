...
class Mario(QWidget):
    def __init__(self):
        ...
        # 창 크기 고정
        self.setFixedSize(self.screen_width + 800, self.screen_height+200)
    ...
    def display_game_info(self):
        self.display_generation = f'{self.ga.generation}세대'
        self.display_which_mario = f'{self.ga.current_chromosome_index + 1}번 째 마리오'
        self.display_fitness = f'{self.ga.chromosomes[self.ga.current_chromosome_index].fitness()}'
        self.display_list = [self.display_generation, self.display_which_mario, self.display_fitness]
    ...
    def paintEvent(self, event):
        ...
        if player_float_state == 0x03 or player_state in (0x06, 0x0B) or player_vertical_screen_position >= 2 or current_chromosome.stop_frames > 180:
            ...
            for i in range(current_chromosome.w2.shape[0]):
                for j in range(current_chromosome.w2.shape[1]):
                    ...
                    #2가 추가되는 이유는 반지름을 12로 줘서 동그라미 중앙을 맞추기 위해 2만큼 더해준 것임
        ...
        #---------------------------------------되는 것-------------------------------------------------------------
        # display_generation = f'== {self.ga.generation} 세대 =='
        # display_which_mario = f'{self.ga.current_chromosome_index + 1}번 마리오'
        # display_fitness = f'{current_chromosome.fitness()}'
        #
        # painter.drawText(self.neural_network_predict_margin_x + self.screen_width + i * 40 + 40, 460, f'{display_generation}{display_which_mario}{display_fitness}')
        #-----------------------------------------안되는 것----------------------------------------------------------------------
        # tt = f"{self.display_generation}"
        # tt = 'Printed value'
        painter.drawText(self.neural_network_predict_margin_x + self.screen_width + i * 40 + 40, 460, self.display_generation)
        painter.end()
