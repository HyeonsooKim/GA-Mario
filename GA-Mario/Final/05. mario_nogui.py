...
#Activation 함수 - def 함수 정의와 같은 방식으로 lambda를 사용해서 함수 정의
...
#왜 여기는 parent class가 추가되어있지 않았지?
class Mario:
    def __init__(self):
        # super().__init__()
        ...
        #nogui함수 실행
        self.nogui()
    #더이상 Timer 함수로 60프레임에 맞춰서 get and set 할 필요가 없기 때문에 while 문 사용해서 알고리즘만 돌림
    def nogui(self):
        while True:
